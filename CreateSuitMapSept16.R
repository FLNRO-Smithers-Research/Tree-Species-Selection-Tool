...
for(k in 1:3){##for each time period
  ...
  ###loop to create maps for each species in treeList###
    ...
    if (nrow(PredSuit) != 0){
      ...
      OrigSuit2 <- OrigSuit
      OrigSuit2$ColCode[!is.na(OrigSuit2$ColCode)] <- "grey"
 #     OrigSuit2 <- spTransform(OrigSuit2, CRS.albers)  # standard albers projection for BC gov't data
 #     OrigSuit2$CurrentSuit <- as.factor(OrigSuit2$CurrentSuit)
      ...
      plot(boundary)
## change cex with map size .6 for District, .08 provincial
      plot(OrigSuit2, col = OrigSuit2$ColCode, pch = 15, cex = 0.6, add = T) ##Current Suit as grey 
      plot(PredSuit, col = as.character(PredSuit$ColCode), pch = 15, cex = 0.6, add = T) ###Future suit - change cex with map size
      ...
      if(k == 1){###only plot current suit once per species
        ...
        png(name2, width = 8, height = 8, units="in", res = 1000)
        plot(boundary)
        plot(OrigSuit, col = as.character(OrigSuit$ColCode), pch = 15, cex = 0.6, add = T)## reset cex size with different map sizes
        plot(boundary, add = TRUE)
        par(xpd=TRUE)
        legend(x = xEx, y = yEx,legend = c("Primary Suitability", "Secondary Suitability","Tertiary Suitability","Becoming Unsuitable","Not Suitable"),
               pch = 22, cex = 1.2, pt.bg = c("green","yellow","red","grey","white"), bty = "n")#, xpd = TRUE
