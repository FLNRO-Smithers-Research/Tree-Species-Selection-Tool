...
#wd=tk_choose.dir()
#setwd(wd)
wd = ("K:/Isabella RScript/Single Tile")
#===============================================================================
...
#fname="BGCv10_2000Pt_Rnd_Normal_1961_1990MSY_RFmodelv4"
...
save(Y1.sub,file=paste(fplot,".Rda",sep=""))
#load("AllGridTest2_90GCMs_MSYT.Rda")
...
Y1.sub$BGC.pred <- predict(BGCmodel, Y1.sub[,-c(1:2,4:6)])
...
Y3x <- as.character(Y2.sub$SiteNo)
Ystr3 <- strsplit(Y3x, "-")
Y5 <- matrix(unlist(Ystr3), ncol=2, byrow=TRUE)
Y2.sub <- cbind(Y5, Y2.sub)
colnames(Y2.sub)[1:2]=c("Tile","TileSite")
write.csv(Y2.sub, file=paste(fplot,"_Predicted", model,".csv",sep=""))
save(Y2.sub, file=paste(fplot,"_Predicted", model,".Rda",sep=""))
#load("AllGridTest2_90GCMs_MSYT_PredictedBioFinal.Rda")
#===============================================================================
...
#FuturePeriod.list <- list(unique(Y2.sub$FuturePeriod))
...
#SiteNo.list <- select.list(unique(Y3.sub1$SiteNo),multiple=FALSE,title="SiteNo", graphics=FALSE)
...
SiteNo.suit <-  foreach(SNL = SiteNo.list, .combine =rbind) %do% {      ##  for each SiteNo in the data
  ...
  SiteFuture.suit <- foreach(i = FuturePeriod.list, .combine = rbind)  %do% {
    ...
    #rm(FTS2, FTS)## ignore warnings to clear variables if working with loop
    
    #SS = "BWBSdk/101"
    
    ## SSlist.reduced <- SSlist[c(1:6,8)]   ## 7 has issues?  "CDFmm/10"
    
    ## SSlist[7]
    
    ## SSlist.reduced <- SSlist[7] 
    
    FTS2 <-  foreach(SS = SSlist, .combine =rbind) %do% {      ##  for each site series for a SiteNo BGC
      ...
      SSfuturelist <- as.list(unique(SSfutureE$SS_NoSpace))
      ...
      cat("Add in species new to site series", "\n")
...
FTS.location = unique (Y2.sub [,names(Y2.sub) %in% c(SiteLocation)])
...
fname <- paste(Tile.pick,"_TreeSuitTraject2",model,".csv",sep= "")
